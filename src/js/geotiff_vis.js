...
async function loadColorizedOverlay(url, is_suit) {
  ...
  const bounds = [
    [13.0210268, -89.32922162], // SW
    [16.46951269, -83.22077558]   // NE
  ];
  var colorizedUrl;
  // --- PNG case ---
  if (url.endsWith(".png")) {
    const colorizedUrl = await colorizePNG(url, is_suit);
    currentLayer = L.imageOverlay(colorizedUrl, bounds, { opacity: 1 }).addTo(map);
    console.log("Colorized PNG loaded:", url);
    return;
  }
    ...
    try {
      ...
      currentLayer = new GeoRasterLayer({
        ...
        pixelValuesToColorFn: (values) => {
          ...
          if (val <= -1) return null; // hide -1 or no data
          const normVal = Math.max(0, Math.min(100, val));
          const color = interpolateColor(normVal, is_suit);
          return color;
        },
      });
      ...
      console.log("GeoTIFF loaded successfully:", url);
    } catch (err) {
      console.error("Error loading GeoTIFF:", err);
      alert("Error loading GeoTIFF file: " + err.message);
    }
  }
}
...
function initializeMap() {
  if (map) return;
  map = L.map("map").setView([14.5, -86.15], 8);
